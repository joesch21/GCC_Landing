...
test('X Stage 2 uses Authorization Code plus PKCE with bounded scopes', () => {
...
test('X Stage 2 can verify identity but cannot create posts', () => {
  ...
  assert.match(source, /token_persistence_enabled:\s*false/);
  assert.match(source, /posting_enabled:\s*false/);
});

test('X Stage 2 public status never exposes secrets', () => {
  ...
  assert.equal(status.token_persistence_enabled, false);
